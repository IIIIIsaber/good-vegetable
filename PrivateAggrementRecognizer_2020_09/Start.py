...
for i in range(10000):
    ...
    youchoice = input(
                      ...
                      "catch pcap:: --------------------------3\n"
                      "Preprocess Pcap:: ---------------------4\n"
                      "deal with data of network:: -----------5\n"
                      "query result of data from database:: --6\n"
                      "Exit:: ------------------------------100\n")
    if youchoice == '1':
        ...
        wrpcap('./pcaps/'+pcap+'.pcap',source)
        print("success!!\n")

    elif youchoice == '4':
        print("You choose pcap process.\n")
        print("Now input the source. \n")
        source = input("source: \n")

        #根据pcap包名生成数据文件名
        data_name_1 = source.split('/')
        data_name_2 = data_name_1[-1].split('.')
        data_name = './data/'+data_name_2[0]+'.csv'
        #******************************************* 数据预处理：*********************************************************************************************
        print("Running!Please wait and donnot do anything!\n")
        try:
            pcapprocess(source=source, Towhere = data_name)
        except:
            print("Error:  Donnot Get the file,maybe you input the wrong file name.")
        else:
            print("Pcap Preprocess Done! \nProduced data in '"+ data_name+"' !\n\n")



    elif youchoice == '5':
        print("You choose using the model to deal with the data of network.\n")
        ...
        try:
            ...
            table_name = 'table_'+table_name_2[0]+'_'+table_name_2[1]
